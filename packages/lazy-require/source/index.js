...
lazyRequire.installAsync = function installAsync(name, opts, next) {
  ...
  if (process.browser === true) {
    error = new Error(
      "lazy-require: installing in the browser is not possible"
    );
    ...
    // Install
    // Arguments
    const args = opts.useNPM ? ["yarn", "add", name] : ["npm", "install", name];
    if (opts.save === true && opts.useNPM) {
      args.push("--save");
