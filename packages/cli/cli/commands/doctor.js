...
module.exports = {
  ...
  run: async function(context) {
    ...
    const nodeVersion = replace(
      'v',
      '',
      await run('node --version', { trim: true })
    )
    ...
    const rnCli =
      rnPath &&
      split(/\s/, await run('react-native --version', { trim: true }))[1] // lulz
    const rnPkg = read(
    ...
    table([[column1('ignite'), column2(igniteVersion), column3(ignitePath)]])
    ...
    const javaVersion =
      javaPath && last(match(/"(.*)"/, await run(javaVersionCmd)))
    ...
    if (isMac) {
      ...
      const xcodeVersion =
        xcodePath &&
        split(/\s/, await run('xcodebuild -version', { trim: true }))[1] // lulz
