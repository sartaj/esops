...
test('resolve stack manifest', t => {
  t.plan(1)
  const actual = parser.resolveStackCompose(MOCK_STACKS.basic)
  const expected = ['../../templates/basic', '../../templates/basic-with-package']
...
test('get list of paths from template directory', t => {
  ...
  const actual = parser.getTemplatePaths(templateDirectory)
  const expected = [
...
test('create patch list from stack manifest', async t => {
  ...
  const stackConfig = parser.resolveStackCompose(MOCK_STACKS.basic)

  const actual = await parser.convertStackComposeToPatchList(stackConfig, MOCK_STACKS.basic)
...
test('check for disallowed duplicate files', async t => {
  ...
  tests.forEach(async ([stack, expected], i) => {
    const stackConfig = parser.resolveStackCompose(stack)
    const patchList = await parser.convertStackComposeToPatchList(stackConfig, stack)
    const actual = parser.validatePatchList(patchList)
...
//     const props = {};
//     // const actual = parser.render(dirname, infrastructureDirectory, props);
//     t.equal(true, true);
//   })
// );
